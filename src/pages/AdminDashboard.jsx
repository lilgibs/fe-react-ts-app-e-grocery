  ...
  const AdminDashboardCard = ({ icon, title, titleDesc, value, bgColor }) => {
    return (
      <div className="h-16 xl:h-24  rounded-md flex flex-row items-center px-3 lg:px-5 gap-2 lg:gap-3 shadow-md text-neutral-500 border">
        <div className=" text-3xl xl:text-6xl">
          {icon}
        </div>
        <div className="flex flex-col">
          <p className="text-neutral-400 text-xs xl:text-base font-semibold ">{title} <span className="text-xs text-pink-500">{titleDesc}</span></p>
      ...
      <div className='flex flex-row min-h-[calc(100vh-64px)] w-full'>
        ...
        <div className="mt-5 rounded-md w-full flex flex-col gap-5">
          <div className="flex flex-row justify-between items-center w-full p-3 lg:p-5 border rounded-md shadow-md">
            <div>
              <p className="font-bold lg:text-xl">Hello, <span className="text-pink-500">{adminGlobal.name}</span></p>
            </div>
            {adminGlobal.role == 99 &&
              <div>
                <Select
                  ...
                  onChange={(selectedOption) => {setSelectedBranchStore(selectedOption ? selectedOption.value : "")
                }}
                />
              </div>
            }
          ...
          <div className="w-full flex flex-col lg:flex-row gap-5">
            <div className="w-full border shadow-md rounded-md flex justify-center px-2 max-h-80">
              {dailySalesData.labels ? (
                <Line data={data.dailySalesData} options={options} />
              ) : (
                <div>Loading...</div>
              )}
            </div>
            <div className="w-full border shadow-md rounded-md flex justify-center px-2 max-h-80">
              {monthlySalesData.labels ? (
                <Line data={data.monthlySalesData} options={options} />
              ) : (
                <div>Loading...</div>
              )}
            </div>
